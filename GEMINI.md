...
See [AGENTS.md](AGENTS.md), [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), and [docs/DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md) for authoritative project guidelines, architecture priorities, testing gates, and UI standards.
...
7. **UI Alignment & Layout Stability**: Never concatenate emoji with text in single strings. Separate emojis with `PlatformTextStyle(includeFontPadding = false)`. Guard multi-metric displays with `maxLines = 1` and `softWrap = false`. In landscape, avoid vertically stacking floating toolbars.
